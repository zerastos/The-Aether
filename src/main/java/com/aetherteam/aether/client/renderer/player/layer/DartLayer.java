...
public class DartLayer<T extends LivingEntity, M extends PlayerModel<T>> extends StuckInBodyLayer<T, M> {
    ...
    public void render(PoseStack poseStack, MultiBufferSource buffer, int packedLight, T livingEntity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        this.random.setSeed((long) (livingEntity.getId() * (0.25D * this.offset)));

        for (int j = 0; j < i; ++j) {
            poseStack.pushPose();
            ModelPart modelPart = this.getParentModel().getRandomModelPart(this.random);
            ModelPart.Cube cube = modelPart.getRandomCube(this.random);
